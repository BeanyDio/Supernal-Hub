...
local settingsTable = {
	General = {
		...
		rayfieldOpen = {Type = 'bind', Value = 'LeftControl', Name = 'Supernal Keybind'},
		-- buildwarnings
		-- rayfieldprompts
...
local RayfieldLibrary = {
	...
	Theme = {
		Default = {
			...
			SliderBackground = Color3.fromRGB(226,0,250),
			SliderProgress = Color3.fromRGB(226,0,250),
			SliderStroke = Color3.fromRGB(226,0,250),

			ToggleBackground = Color3.fromRGB(30, 30, 30),
			ToggleEnabled = Color3.fromRGB(214,0,214),
			ToggleDisabled = Color3.fromRGB(100, 100, 100),
			ToggleEnabledStroke = Color3.fromRGB(177,0,255),
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and not Icons then
		warn("Supernal | Cannot use Lucide icons as icons library is not loaded")
	else
		warn("Supernal | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
	end
	return assetUri
end
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('Supernal had an issue decoding the configuration file, please try delete the file and reopen Supernal.') return end
	...
	for FlagName, Flag in pairs(RayfieldLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn("Supernal | Unable to find '"..FlagName.. "' in the save file.")
	...
	if Settings.Discord and not useStudio then
		...
		if isfile and not isfile(RayfieldFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(RayfieldFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"Supernal RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
